...
AForm::AForm()
    : name("DefaultForm"), grade_sign(150), grade_exec(150), indicateur(false)
{
}

AForm::AForm(const std::string& name, int grade_sign, int grade_exec)
    : name(name), grade_sign(grade_sign), grade_exec(grade_exec), indicateur(false)
{
    if (this->grade_sign > 150 || this->grade_exec > 150)
        throw AForm::GradeTooLowException();
    else if (this->grade_sign < 1 || this->grade_exec < 1)
...
AForm::AForm(const AForm &other)
    : name(other.name), grade_sign(other.grade_sign), grade_exec(other.grade_exec), indicateur(other.indicateur)
{
}
...
{
    out << f.getName()
        << ", AForm grade required to sign it: " << f.getGrade_sign()
        << ", grade required to execute it: " << f.getGrade_exe();
    return out;
}
